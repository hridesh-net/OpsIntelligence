---
name: drawio-skill
description: Use when user requests diagrams, flowcharts, architecture charts, visualizations, multi-page or satellite diagrams, reference blocks for detail views, connectivity-only topology maps, or non-overlapping edge routing. Also use proactively when explaining systems with 3+ components, complex data flows, or relationships that benefit from visual representation. Generates .drawio XML files and exports to PNG/SVG/PDF locally using the native draw.io desktop CLI.
license: MIT
...
## Diagram clarity and composition (required)

Apply these rules when planning, generating, or revising any flow or architecture diagram unless the user explicitly overrides them.

### Primary reading direction (flow)

- **Default story order is left-to-right (LTR):** place triggers, ingress, users, or sources on the **left**; sinks, outcomes, persistence, or consumers on the **right**.
- After the main LTR spine exists, **branches may turn** (vertical segments, short back-edges) when canvas size or grouping demands it. Do **not** lay out the primary narrative so it reads right-to-left by default.

### Edge and line hygiene

- **Edges must not visually overlap** each other or cut through unrelated shapes. Treat overlapping lines as a defect.
- Prefer orthogonal edges, **distribute** `exitX` / `exitY` / `entryX` / `entryY` across the shape perimeter when several edges share a node, add `<Array as="points">` waypoints, and increase spacing until routes are distinct (see Step 5 self-check).

### Multi-diagram and reference blocks

- **Internal functioning of one component:** use a **separate mini-diagram** (satellite region **above or to the left** of the main canvas, or an extra **diagram tab** in the same `.drawio` file). In the **main** diagram, use a **reference block/card** (name + short pointer, e.g. “Detail: Sync pipeline — see tab *Semantic ingest*”) so readers know where to look.
- **Parallel flows that never depend on each other** and do not affect sequencing: show each in its **own** diagram (another tab, another file, or a clearly separated satellite frame) and use **reference blocks** in the main view only where they aid navigation—avoid one canvas of unrelated spaghetti.
- **Multiple independent “main” stories:** prefer **separate** diagrams (pages/files); link them with titles and reference cards rather than merging into a single unreadable graph.

### Connectivity-only diagrams

- When the goal is **component connectivity** (what connects to what) **without** control flow or step order, create a **separate** `.drawio` file or page that is **topology-oriented:** nodes, links, optional port labels; **do not** imply temporal order unless edges are explicitly labeled (e.g. bidirectional, protocol name).
...
2. **Plan** — identify shapes, relationships, layout, group by tier/layer. **Default the main flow to LTR** (see **Diagram clarity and composition**). Decide early whether independent flows or heavy internal detail belong in **satellite regions, extra tabs, or separate `.drawio` files** with **reference blocks** in the main view; plan a **connectivity-only** sibling file if the user asks for topology without sequencing.
...
| Unrelated flows merged | Two or more independent main stories on one canvas | Split into separate tabs/files; add reference blocks in the main diagram |
| Primary flow reads RTL | Main narrative goes right-to-left by default | Re-layout main spine LTR; keep exceptions to short branches only |
