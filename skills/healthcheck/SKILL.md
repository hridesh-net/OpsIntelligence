---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpsIntelligence deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpsIntelligence cron scheduling for periodic checks, or version status checks on a machine running OpsIntelligence (laptop, workstation, Pi, VPS).
---

# OpsIntelligence Host Hardening

## Overview

Assess and harden the host running OpsIntelligence, then align it to a user-defined risk tolerance without breaking access. Use OpsIntelligence security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpsIntelligence changes the host firewall, SSH, or OS updates; it does not.
...
5. OpsIntelligence gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpsIntelligence can access sensitive data.
...
### 2) Run OpsIntelligence security audits (read-only)

As part of the default read-only checks, run `opsintelligence security audit --deep`. Only offer alternatives if the user requests them:

1. `opsintelligence security audit` (faster, non-probing)
2. `opsintelligence security audit --json` (structured output)

Offer to apply OpsIntelligence safe defaults (numbered):

1. `opsintelligence security audit --fix`

Be explicit that `--fix` only tightens OpsIntelligence defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpsIntelligence version/update status (read-only)

As part of the default read-only checks, run `opsintelligence update status`.
...
- Credential hygiene notes (location of OpsIntelligence creds, prefer disk encryption)
...
- OpsIntelligence security audit (re-run)
...
After OpsIntelligence install or first hardening pass, run at least one baseline audit and version check:

- `opsintelligence security audit`
- `opsintelligence security audit --deep`
- `opsintelligence update status`

Ongoing monitoring is recommended. Use the OpsIntelligence cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `opsintelligence cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `opsintelligence update status`
...
Before creating, `opsintelligence cron list` and match on exact `name`. If found, `opsintelligence cron edit <id> ...`.
If not found, `opsintelligence cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `opsintelligence update status` (preferred for source checkouts and channels)
2. `npm view opsintelligence version` (published npm version)

## OpsIntelligence command accuracy

Use only supported commands and flags:

- `opsintelligence security audit [--deep] [--fix] [--json]`
- `opsintelligence status` / `opsintelligence status --deep`
- `opsintelligence health --json`
- `opsintelligence update status`
- `opsintelligence cron add|list|runs|run`

Do not invent CLI flags or imply OpsIntelligence enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpsIntelligence compaction:
